...
int BaseHandler::sendSimplyResp(const char *uname, struct eXosip_t *excontext, int tid, int status)
{
...
int BaseHandler::sendCallAck(struct eXosip_t *excontext, int did)
{
    eXosip_lock(excontext);
...
int BaseHandler::getStatcodeFromResp(osip_message_t *resp)
{
    return resp != nullptr ? resp->status_code : -1;
}

const char *BaseHandler::getMsgIdFromReq(osip_message_t *req)
{
